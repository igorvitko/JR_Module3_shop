...
```bash
uv run pytest        # тести
uv run pytest --cov --cov-report=term-missing   # тести з покриттям
...
- [x] Етап 13 — Якість коду: повна типізація методів, покриття тестами розширено (+7 тестів), coverage у CI, mypy-чисто (тести послаблено навмисно — factory-boy без стабів)
- [ ] Етап 14 — Продакшн-інфраструктура (Docker prod, nginx, збірка React) та фінальний README
